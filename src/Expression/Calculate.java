 ...
 *
 * @github https://github.com/zorenv https://github.com/panwei1995
 *
 * @email hit_lmf@163.com 492358046@qq.com
 *
 *        All right reserved.
 *
 */
	...
	public static void main(String[] args) {
		...
		while (!test.equals("end")) {
			...
			if (test.equals(""))
				...
				continue;
			} else if (!test.equals("end") || (test.charAt(0) >= '1' && test.charAt(0) <= '9')
					|| (test.charAt(0) >= 'a' && test.charAt(0) <= 'z')
					|| (test.charAt(0) >= 'A' && test.charAt(0) <= 'Z') || test.charAt(0) == ' ') {
				for (i = 0; i < test.length(); i++) {
					if (!((test.charAt(i) >= '0' && test.charAt(i) <= '9')
							|| (test.charAt(i) >= 'a' && test.charAt(i) <= 'z')
							|| (test.charAt(i) >= 'A' && test.charAt(i) <= 'Z') || test.charAt(i) == ' '
...
/*
 * 测试1 给全部变量赋值，并得到最终计算结果 30*2+6*8*x*x+9*x*y !simplify x=2 y=6 !d/dz
 * 
 * 测试 2 接收空格與Tab 30 *2+ 6*8*x*x +9 *x*y !simplify x=2 !d/dx
 * 
 */
